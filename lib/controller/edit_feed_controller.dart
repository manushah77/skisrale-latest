...
class EditFeed_Controller extends GetxController {
  RxString token = "".obs;
  RxString id = "".obs;
  var data = [].obs;
  var likedPosts = [].obs;
  ...
  Future fetchFeeds(int feedId) async {
    try {
      isLoading(true);
      SharedPreferences prefs = await SharedPreferences.getInstance();
      // setState(() {
      ...
      // Make the API call
      var response = await UserService().postApiwithToken("api/feed_detail", data, token.toString());

      if (response["status"] == "success") {
        print('asdasdadadasdadaad');
        print(response);
        return response;
        // Handle success response
        // You can update your data or do something else here
      } else {
